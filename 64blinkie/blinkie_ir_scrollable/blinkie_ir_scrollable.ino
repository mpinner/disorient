...
String letters = "crashs";

int maxFrames = (letterCount * (8 + 1)) + 1;
...
long interval = 40; //duration to display each letter;
...
void loop() {
  ...
  if(currentMillis - previousMillis > interval) {
    ...
    currentFrame++; //increment letter
    if (currentFrame == maxFrames) {
      currentFrame = 0; // or repeat
    }
...
void displayFrame(int frame) {
  ...
  int centerLetterIndex = (frame / 9);
  int firstLetterIndex = (centerLetterIndex == 0) ? letterCount : centerLetterIndex - 1;
  int lastLetterIndex = (centerLetterIndex == letterCount) ? 0 : centerLetterIndex + 1;


  char frames[frameCount];
  for  (int i=0; i<frameCount; i++) {
    frames[i] = 0x00;    
  } 
  
  // copy in first letter
  for  (int i=0; i<8; i++) {
    frames[i] = font[letters[firstLetterIndex]][i];    
  } 
  
  // copy in center letter
  for  (int i=0; i<8; i++) {
    frames[i+9] = font[letters[centerLetterIndex]][i];    
  } 
  
  // copy in last letter
  for  (int i=0; i<8; i++) {
    frames[i+18] = font[letters[lastLetterIndex]][i];    
  } 
  ...
  // copy in last letter
  for  (int i=0; i<8; i++) {
    codedLetter[i] = frames[frame%9 + (frameCount/2) +i];    
  } 
  
  displayEncodedLetter(codedLetter); 
